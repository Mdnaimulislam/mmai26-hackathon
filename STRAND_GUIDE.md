...
- **Thermal mass and flats** — Flats share walls, floors, and ceilings with neighbouring dwellings. Heat conducted from adjacent properties means flats retain warmth more efficiently than detached or semi-detached houses in the same postcode. In winter months, flats can be 2–3°C warmer than comparable detached houses. A model without property-type features may systematically over-predict cold risk for flats, wasting upgrade budget, while under-predicting for detached houses, where the real risk is concentrated.

- **Why CO₂ sensors fail** — Sensor hardware degrades and goes offline. The dataset shows that the 40 properties with the heaviest CO₂ dropout are disproportionately older, in higher-deprivation postcodes, and in worse structural condition. Sensor failure is not random — it correlates with the very factors that increase cold risk. Any model trained on CO₂ data will be least reliable for the households most likely to need an upgrade: this is the MNAR problem in the housing context.
