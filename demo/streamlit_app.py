"""Streamlit app for Housing strand demo solution."""
...
"""
    ...
    c1.metric("Merged rows", result["merged_rows"])
    c2.metric("Train rows", result["train_rows"])
    c3.metric("Test rows", result["test_rows"])
    ...
    st.warning(
        ...
        "`reference/housing_benchmark_card.json` with their own evidence-backed narratives, answers, and verdicts."
    )
    st.markdown(
        """
...
"""
    ...
    train_path = metrics_dir / "training_metrics.csv"
    ...
    if train_path.exists():
        st.markdown("`training_metrics.csv`")
        st.dataframe(pd.read_csv(train_path), use_container_width=True)
